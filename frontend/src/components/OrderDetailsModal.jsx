...
export default function OrderDetailsModal({ order, onClose }) {
  ...
  const address = details?.shippingAddress || details?.address || details?.deliveryAddress || details?.customerAddress;
  const formatAddress = (addressObj) => {
    ...
    return [
      ...
      addressObj.city,
      addressObj.state,
      addressObj.postalCode || addressObj.zipCode || addressObj.pincode,
      addressObj.country,
    ]
      .filter(Boolean)
  ...
  return <div className="modal-overlay" onClick={onClose}><div className="modal-content" onClick={(event) => event.stopPropagation()} style={{ maxWidth: '760px', maxHeight: '85vh', overflowY: 'auto', textAlign: 'left' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '1rem', margin: '1rem 0' }}>
      ...
      <div><strong><MapPin size={15} style={{ verticalAlign: 'middle' }} /> Shipping address</strong><p>{formatAddress(address)}</p></div>
    </div>
